"""
This file contains the CATVI for Gamma_DP model
"""
...
from src.CATVI import lda_e_step
...
class Global_Prior():
    ...
    def update_G_0(self, rhot, mat_phi, chunk_doc_words_no):
        self.mat_phi = np.copy(mat_phi)
        
        self.new_G_0 = np.ones(self.m_K+1)/(self.m_K+1) 
        
        self.vector = np.sum(self.mat_phi, axis=1) * self.m_mu * self.new_G_0         * self.m_D/self.chunksize 

        self.vector[0] += self.m_alpha

        self.new_G_0 = self.vector/np.sum(self.vector)
           
        self.mu_matrix = digamma(self.m_mu)-digamma(self.m_mu + chunk_doc_words_no)
        
        self.mu_gradient = -1+(self.m_alpha)/self.m_mu +         self.m_D/self.chunksize*(np.sum(self.mu_matrix + np.dot(self.G_0, self.mat_phi)))

        old_mu = self.m_mu + 0
        self.m_mu += rhot * self.mu_gradient/(self.m_D/self.chunksize) * 0.1
        self.m_mu = min(max(1, self.m_mu), old_mu+5)
...
class HdpModel_CATVI():
    ...
    def update_chunk(self, chunk):
        ...
        self.G_0.update_G_0(self.rhot, self.mat_phi[self.effe_list], np.sum(self.mat_z_sum[i][self.effe_list], axis=0))
        self.m_gamma = self.G_0.m_mu + 0 
    ...
    def update_lambda(self, rhot):
        ...
        for i in range(self.chunk_doc_no):
            ids = self.chunk_doc_word_ids_list[i]
            cts = self.chunk_doc_word_counts_list[i]
            self.m_lambda[np.ix_(self.effe_list, ids)] += rhot* (self.m_D /self.chunksize)            * (np.tile(cts, (self.m_K+1, 1)) *  self.mat_z_avrg[i][self.effe_list]) #self.m_updatect+1
...
if __name__ == '__main__':
    ...
    save_path = directory + '/result_catvi_gamma'
    ...
    np.save(save_path + '/G_0_gamma', model.G_0.G_0)
    
    model.result.index.name = 'iter_no'
    model.result.to_csv(save_path + '/cvi_gamma.csv')
    ...
    Topics.to_csv(save_path + '/topics_gamma.csv')  
